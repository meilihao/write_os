...
void hal_run_intflthandle(uint_t ifdnr, void *sframe)
{
    ...
    //遍历i_serlist链表
    //Cosmos 链表借用了 Linux 所用的链表，代码我已经帮你写好了，放在了 list.h 和 list_t.h
    list_for_each(lst, &ifdscp->i_serlist)
    {
        isdscp = list_entry(lst, intserdsc_t, s_list); //获取i_serlist链表上对象即intserdsc_t结构
        isdscp->s_handle(ifdnr, isdscp->s_device, sframe); //调用中断处理回调函数 
    }
