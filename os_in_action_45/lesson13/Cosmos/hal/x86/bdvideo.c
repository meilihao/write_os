...
// 初始化图形显示驱动，内核在运行过程要在屏幕上输出信息
PUBLIC LKINIT void init_bdvideo()
{
    dftgraph_t *kghp = &kdftgh;

    init_dftgraph(); // 初始化图形数据结构，里面放有图形模式，分辨率，图形驱动函数指针
    init_bga(); //初始bga图形显卡的函数指针
    init_vbe(); //初始vbe图形显卡的函数指针
    fill_graph(kghp, BGRA(0, 0, 0)); //清空屏幕 为黑色
    set_charsdxwflush(0, 0); //显示背景图片 
    hal_background();
