...
// 内存分割合并数据结构——memdivmer_t
// 分配内存就是分割内存，而释放内存就是合并内存
typedef struct s_MEMDIVMER
{
	spinlock_t dm_lock; //保护自身结果的自旋锁
	u32_t dm_stus; //状态
	uint_t dm_dmmaxindx;
	uint_t dm_phydmindx;
	uint_t dm_predmindx;
	...
	bafhlst_t dm_mdmlielst[MDIVMER_ARR_LMAX]; //bafhlst_t结构数组, 通过 dm_mdmlielst 数组，来划分物理内存地址不连续的 msadsc_t 结构
...
// 每个内存区 memarea_t 结构中包含一个内存分割合并 memdivmer_t 结构，而在 memdivmer_t 结构中又包含 dm_mdmlielst 数组. 在 dm_mdmlielst 数组中挂载了多个 msadsc_t 结构
typedef struct s_MEMAREA //内存区的数据结构
{
