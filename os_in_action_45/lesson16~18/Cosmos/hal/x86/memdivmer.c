...
msadsc_t *mm_divpages_opmsadsc(msadsc_t *msastat, uint_t mnr)
{
	...
	if (mend == msastat) //单个msadsc_t结构的情况 
...
// 若是相等的，则在 mm_reldpgsdivmsa_bafhl 函数中很容易处理，只要取出 bafhlst_t 结构中对应的 msadsc_t 结构返回就好了
// 不相等，这就要分隔连续的 msadsc_t 结构即mrdmb_add_msa_bafh
bool_t onmpgs_retn_bafhlst(memarea_t *malckp, uint_t pages, bafhlst_t **retrelbafh, bafhlst_t **retdivbafh)
{
	...
	for (sint_t idx = dividx; idx < MDIVMER_ARR_LMAX; idx++)
	{
		...
		{
			*retrelbafh = &bafhstat[dividx]; //返回请求分配的bafhlst_t结构指针
			*retdivbafh = &bafhstat[idx]; //返回实际分配的bafhlst_t结构指针
			return TRUE;
		}
	}
