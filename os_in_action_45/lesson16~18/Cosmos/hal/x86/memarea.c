...
bool_t merlove_mem_onmemarea(memarea_t *mareap, msadsc_t *mstat, uint_t msanr)
{
	...
	for (; fntmnr < msanr;)
	{
		//获取最多且地址连续的msadsc_t结构体的开始、结束地址、一共多少个msadsc_t结构体，下一次循环的fntmnr即 msadsc_t 结构体的索引号
...
bool_t merlove_mem_core(machbstart_t *mbsp)
{
	...
	//遍历每个memarea_t结构:当确定内存页属于哪个区之后, 合并其中的 msadsc_t 结构，并把它们挂载到其中的 memdivmer_t 结构下的 dm_mdmlielst 数组中
