...
void write_pixcolor(machbstart_t *mbsp, u32_t x, u32_t y, pixl_t pix)
{
    ...
    phybas[x + (y * mbsp->mb_ghparm.gh_x)] = pix; // x，y是像素的位置
...
// 在bmp格式的文件中，除了文件头外的数据就是图形像素点的数据，只不过 24 位的位图每个像素占用 3 字节，并且位置是倒排的，即第一个像素的数据是在文件的最后，依次类推。我们只要依次将位图文件的数据，按照倒排次序写入显存中，这样就可以显示了
...
// 在计算机加电启动时，计算机上显卡会自动进入文本模式，文本模式只能显示 ASCII 字符，不能显示汉字和图形，所以要让显卡切换到图形模式
void init_graph(machbstart_t *mbsp)
{
    ...
    if (mbsp->mb_ghparm.gh_mode != BGAMODE)
    {
        ...
        // 这里选择使用了 VBE 的 118h 模式，该模式下屏幕分辨率为 1024x768，显存大小是 16.8MB. 显存开始地址一般为 0xe0000000
        // 屏幕分辨率为 1024x768，即把屏幕分成 768 行，每行 1024 个像素点，但每个像素点占用显存的 32 位数据（4 字节，红、绿、蓝、透明各占 8 位）. 只要往对应的显存地址写入相应的像素数据，屏幕对应的位置就能显示了
        // 下面3函数均通过bios中断
        get_vbemode(mbsp);     // 获取VBE模式 
        get_vbemodeinfo(mbsp); // 获取一个具体的VBE模式的信息
        set_vbemodeinfo();     // 设置VBE模式
    }
